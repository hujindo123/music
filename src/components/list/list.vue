  ...
  <div class="list">
    ...
    <ul class="list-data" v-if="recommend">
      <template v-for="(item, index) in recommend">
        <!-- 推荐歌单 -->
        <li v-if="item.type===0" class='type-0'>
          <img :src="item.picUrl" alt="">
          <p class="list-name">{{item.name}}</p>
          <span class="type-0-span"><i class="icon-erji"></i> {{(parseInt(item.playCount) / 10000 < 10) ? item.playCount : (item.playCount / 10000).toFixed(0)+'万'}}</span>
        </li>
        <!-- 独家放送 -->
        <li v-if="item.type===24 || item.type===21" :class="index < 2 ? 'type-24': 'type-21'">
          <img :src="item.picUrl" alt="">
          <p class="list-name">{{item.name}}</p>
          <span  class="type-24-span"><i class="icon-mv"></i></span>
        </li>
        <!-- 最新音乐 -->
        <li v-if="item.type===4"  :class="[{'type-4': item.type===4}]">
        ...
        <!-- 推荐mv -->
        <li v-if="item.type===5" class="type-5">
          <img :src="item.picUrl" alt="">
          <p class="titles">{{item.name}}</p>
          <span  class="type-5-span"><i class="icon-mv"></i> {{(parseInt(item.playCount) / 10000 < 10) ? item.playCount : (item.playCount / 10000).toFixed(0)+'万'}}</span>
